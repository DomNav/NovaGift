...
export function WalletGate({
  ...
  const { publicKey, connected, isInstalled, connecting, connect, disconnect } = useWallet();
  ...
  if (!isInstalled) {
    return (
      <div className="wallet-gate">
        ...
        {fallbackMode === 'demo' ? (
          <div className="demo-mode-content opacity-75">{children}</div>
        ) : (
          <div className="disabled-content opacity-50 pointer-events-none">{children}</div>
        )}
      </div>
    );
  }
    ...
    return (
      <div className="wallet-gate">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
          ...
          <p className="text-gray-600 mb-4">Connect your Freighter wallet to access all features</p>
          <button
            ...
            {connecting ? (
              ...
              <>
                <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                  <path
                    fillRule="evenodd"
                    d="M4.083 9h1.946c.089-1.546.383-2.97.837-4.118A6.004 6.004 0 004.083 9zM10 2a8 8 0 100 16 8 8 0 000-16zM9.954 4.569c-1.548 0-2.802 1.662-2.802 3.712 0 2.05 1.254 3.712 2.802 3.712 1.547 0 2.802-1.662 2.802-3.712 0-2.05-1.255-3.712-2.802-3.712z"
                    clipRule="evenodd"
                  />
                </svg>
                Connect Freighter
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-green-600 text-sm font-mono">{formatAddress(publicKey)}</span>
          </div>
          <button
